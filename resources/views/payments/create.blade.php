@extends('layouts.app')
@php
    $category = \App\Models\FeeCategory::find(old('fee_category_id') ?? request('fee_category_id'));
@endphp
...
<div class="max-w-3xl mx-auto py-8">
    ...
    <form action="{{ route('payments.store') }}" method="POST" class="bg-white p-6 rounded shadow">
        ...
        <div class="mb-4">
            ...
            <select name="student_id" id="student_id" class="w-full border border-gray-300 rounded px-3 py-2 mt-2" required style="display: none;">
                ...
                @foreach($students as $student)
                    <option value="{{ $student->id }}" data-name="{{ strtolower($student->name) }}" data-number="{{ $student->student_number }}" {{ old('student_id') == $student->id ? 'selected' : '' }}>
                ...
                @for ($i = 1; $i <= 12; $i++)
                    <label class="inline-flex items-center">
                        <input type="checkbox" name="months[]" value="{{ $i }}" class="form-checkbox text-blue-600"
                            {{ is_array(old('months')) && in_array($i, old('months')) ? 'checked' : '' }}>
                        <span class="ml-2">
        ...
        <div class="mb-4">
            ...
            <input type="number" name="amount" id="amount" value="{{ old('amount') }}" class="w-full border border-gray-300 rounded px-3 py-2" required>
            @error('amount')
                <div class="text-red-500 text-sm mt-1">{{ $message }}</div>
            @enderror
        </div>
